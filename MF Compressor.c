/*
  ...
  Copyright: You can use it anyway you want.
...
void decompress(FILE *fw,char *str)
{
	  ...
	  {
		  temp[0]=str[i]+115;// it was 114 before
		  printf("\n%c = %d",temp[0],str[i]);
		  temp[1]='\0';
		  count=atoi(temp);
		...
		case '1':
		     {
			      ...
			      case 'Y':
				 {
				     ...
				     p("\nTo view the file press 'V' or anyother key to quit: ");
				     char v=getche();
				     if ( v== 'v' || v=='V')
				     {
					puts(file_loc);
